...
export function Dashboard({ onNavigate }: { onNavigate: (page: string) => void }) {
  ...
  const activeCarbs = carbsPercent * animProgress;
  const activeProtein = proteinPercent * animProgress;
  const activeFat = fatPercent * animProgress;
      ...
      <section className="grid gap-4 lg:grid-cols-[minmax(0,1.45fr)_minmax(330px,1fr)]">
        ...
        <article className={`${panelClassName} flex flex-col p-5`}>
          ...
          <div className="flex flex-1 flex-wrap items-center justify-around gap-7 py-4">
            <div className="grid h-44 w-44 place-items-center rounded-full transition-all duration-500" style={{ background: macroTotal ? `conic-gradient(#f2ad20 0 ${activeCarbs}%, #146146 ${activeCarbs}% ${activeCarbs + activeProtein}%, #dc584e ${activeCarbs + activeProtein}% ${activeCarbs + activeProtein + activeFat}%, #e7e4dc ${activeCarbs + activeProtein + activeFat}% 100%)` : "#e7e4dc" }}><span className="grid h-28 w-28 place-items-center rounded-full bg-[#fffdf8] text-center"><span><strong className="block font-serif text-3xl font-normal">{nutrition.calories.toLocaleString()}</strong>kcal{calorieGoal ? <small className="block text-[10px] text-[#70726d]">goal {calorieGoal.toLocaleString()}/day</small> : null}</span></span></div>
        ...
        <article className={`${panelClassName} p-4`}>
          ...
          {summary.recommendedRecipes.length ? <div className="space-y-2">{summary.recommendedRecipes.slice(0, 3).map((recipe) => (
            <div className="grid items-center overflow-hidden rounded-xl border border-[#e4daca] bg-white/25 sm:grid-cols-[200px_1fr_auto_auto] transition-all duration-300 hover:-translate-y-1 hover:border-[#bdae9c] hover:bg-white hover:shadow-[0_8px_20px_rgba(30,70,50,0.08)]" key={recipe.recipeId}>
