...
export function MyRecipes({ onNavigate }: ScreenProps) {
  ...
  return <DashboardPageShell activePage="My Recipes" menuOpen={menuOpen} onMenuOpenChange={setMenuOpen} onNavigate={onNavigate}>
    ...
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{recipes.map((recipe) => <article key={recipe.id} className="overflow-hidden rounded-2xl border border-[#ded5c5] bg-[#fffdf8] shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-[#bdae9c] hover:shadow-[0_12px_24px_rgba(5,54,42,0.08)]">
